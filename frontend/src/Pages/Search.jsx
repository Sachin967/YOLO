  ...
  return (
    <div className="h-screen flex">
      <div className="ml-20 w-[694px] relative md:w-[1110px] lg:w-[750px] min-h-screen max-h-full sm:w-[980px] lg:ml-[320px] sm:ml-[55px] bg-white dark:bg-black">
        ...
        {recentSearches.length > 0 && <div className="flex">
          <h1 className="p-4 pl-7 text-lg font-semibold dark:text-white text-black">Recent</h1>
          <div className="ml-auto">
            <h1 onClick={() => {
              setUser([]);
              setRecentSearches([]);
            }} className="text-base cursor-pointer font-semibold text-blue-700 pr-5 mt-5">Clear All</h1>
          </div>
        </div>}
        <div >
          {user && user.map((u) => (
          ...
          {user?.length === 0 && search === "" && (
            <>{recentSearches.length == 0 && <div className="flex items-center justify-center mt-5"><h1 className="text-gray-400 font-semibold">No Recent Searches</h1></div>}
              <div >
                {recentSearches.map((recentUser) => (
                  <>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <h1 className="text-lg dark:text-white text-black font-semibold">{recentUser?.name}</h1>
                        <h1 className="text-gray-500">{recentUser?.username}</h1>
                      </div>
                      <button onClick={(e) => RemoveUser(e, recentUser)} className="cursor-alias dark:text-white text-black text-2xl pe-5"><MdClear /></button>
                    </div>
                  </>
                ))}
              </div></>
